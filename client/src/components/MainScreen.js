...
const MainScreen = () => {
    ...
    const onSubmit = async evt => {
        ...
        console.log("Ayo response check: ", response)
        setPlayerStatsState({ submit: true, matchId: response.data._id, player1: response.data.players[0], player2: response.data.players[1], player3: response.data.players[2], player4: response.data.players[3] })   
    }
    useEffect (()=>{
        if(playerStatsState){
